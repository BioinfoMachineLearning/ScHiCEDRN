...
class Hiedsr_2D(pl.LightningModule):
    ...
    def G_loss(self, out_labels, output, target):  # for total Hiedsrgan three parameters
    # for Hiedsr only two parameters
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            mse, perception, tv, loss = self.G_loss(pred_fake.mean(), fake_image, target)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        label_fake = torch.ones_like(pred_fake)
        mse, perception, tv, loss = self.G_loss(pred_fake.mean(), fake_image, target)
        self.log('carn_validation_Gloss/total', loss)
        ...
        d_loss_real = self.Dloss(pred_real, label_real)
        total_loss = d_loss_fake + d_loss_real
        self.log('carn_validation_Dloss/loss', total_loss)
        return loss
