...
def loadBothConstraints(stria, strib, res, percent):
    ...
    # print("\n=========== Permutation method the contact map shape: {} and data length is {}".format(contact_mapb_dele.shape, len(contact_mapb_dele)))
    #  print(contact_mapb_dele)
    ...
    def extract_constraint_mats(self):
        ...
        for i in c1.index:
            ...
            if i == 6:
                ...
                c2.to_csv('DataFull'+str(self.percent)+'/Constraints/chrom_' + str(i + 1) + '_' + str(self.res) + '.txt', sep = '\t', index = False, header = False)

    def extract_create_numpy(self):
        if not os.path.exists("DataFull"+str(self.percent)+"/Full_Mats"):
            subprocess.run("mkdir -p DataFull"+str(self.percent)+"/Full_Mats", shell = True)

        globs = glob.glob("DataFull"+str(self.percent)+"/Constraints/chrom_1_" + str(self.res) + ".txt")
        if len(globs) == 0:
        ...
        for i in range(1, 7):
            target, data = loadBothConstraints(
                "DataFull"+str(self.percent)+"/Constraints/chrom_" + str(i) + "_" + str(self.res) + ".txt",
                "DataFull"+str(self.percent)+"/Constraints/chrom_" + str(i) + "_" + str(self.res) + ".txt",
            ...
            if i == 5:
                ...
                print(target)
            # input("Press Enter to continue...")
            np.save("DataFull"+str(self.percent)+"/Full_Mats/GSE131811_mat_full_chr_" + str(i) + "_" + str(self.res), target)
            np.save("DataFull"+str(self.percent)+"/Full_Mats/GSE131811_mat_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res), data)

    def split_numpy(self):
        if not os.path.exists("DataFull"+str(self.percent)+"/Splits"):
            subprocess.run("mkdir -p DataFull"+str(self.percent)+"/Splits", shell = True)

        globs = glob.glob("DataFull"+str(self.percent)+"/Full_Mats/GSE131811_mat_full_chr_1_" + str(self.res) + ".npy")
        if len(globs) == 0:
            self.extract_create_numpy( )

        for i in range(1, 7):
            target = splitPieces("DataFull"+str(self.percent)+"/Full_Mats/GSE131811_mat_full_chr_" + str(i) + "_" + str(self.res) + ".npy",
                                    self.piece_size, self.step)
            data = splitPieces("DataFull"+str(self.percent)+"/Full_Mats/GSE131811_mat_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res) + ".npy",
                                  self.piece_size, self.step)
            ...
            if (i == 5):
                ...
                # input("Press Enter to continue...")
            np.save(
                "DataFull"+str(self.percent)+"/Splits/GSE131811_full_chr_" + str(i) + "_" + str(self.res) + "_piece_" + str(self.piece_size),
                target)
            np.save(
                "DataFull"+str(self.percent)+"/Splits/GSE131811_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res) + "_piece_" + str(self.piece_size),
                data)

    def prepare_data(self):
        print("Preparing the Preparations ...")
        globs = glob.glob(
            "DataFull"+str(self.percent)+"/Splits/GSE131811_full_chr_*_" + str(self.res) + "_piece_" + str(self.piece_size) + str(".npy"))
        if len(globs) > 5:
            print("Ready to go")
        else:
    ...
    class gse131811Dataset(Dataset):
        def __init__(self, full, tvt, res, piece_size, percent):
            ...
            if full == True:
                ...
                self.target = np.load(
                    "DataFull"+str(self.percent)+"/Splits/GSE131811_full_chr_" + str(self.chros[0]) + "_" + str(self.res) + "_piece_" + str(
                        self.piece_size) + ".npy")
                self.data = np.load(
                    "DataFull"+str(self.percent)+"/Splits/GSE131811_" + str(self.percent) + "_chr_" + str(self.chros[0]) + "_" + str(self.res) + "_piece_" + str(
                        self.piece_size) + ".npy")
                self.info = np.repeat(self.chros[0], self.data.shape[0])
                for c, chro in enumerate(self.chros[1:]):
                    temp = np.load(
                        "DataFull"+str(self.percent)+"/Splits/GSE131811_full_chr_" + str(chro) + "_" + str(self.res) + "_piece_" + str(
                            self.piece_size) + ".npy")
                    ...
                    temp = np.load(
                        "DataFull"+str(self.percent)+"/Splits/GSE131811_" + str(self.percent) + "_chr_" + str(chro) + "_" + str(self.res) + "_piece_" + str(
                            self.piece_size) + ".npy")
                    if len(temp) == 0:
                        pass
                    else:
...
if __name__ == "__main__":
    ...
    obj = GSE131811Module(percent = 1.0)
    obj.prepare_data()
    obj.setup(stage = 'fit')
